...
export async function listPools(options?: {
  ...
  // The API paginates by *groups* (token pair names like "SOL-USDC"), not
  // individual pools. Each group contains many pools with different bin steps.
  // We request enough groups to exceed the desired pool count, then flatten.
  ...
  // Sort by liquidity and slice to the requested limit
  pools = pools
